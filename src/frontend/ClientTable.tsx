...
export default function ClientTable({
  ...
  const handleCreateChange = (e: {
    ...
    if (
      (name === "identificacion" || name === "telefono") &&
      !/^\d*$/.test(value)
    ) {
      if (!errorFlag.current) {
        errorFlag.current = true; // Set the flag to true to prevent duplicate alerts
        notifyError(
          `Solo se permiten números en el campo ${
            name === "identificacion" ? "identificación" : "teléfono"
          }.`
        );
    ...
    if (
      (name === "nombre" || name === "direccion") &&
      !/^[a-zA-Z0-9\sáéíóúÁÉÍÓÚñÑ.,-]*$/.test(value)
    ) {
      if (!errorFlag.current) {
        errorFlag.current = true; // Set the flag to true to prevent duplicate alerts
        notifyError(
          `No se permiten caracteres especiales en el campo ${
            name === "nombre" ? "nombre" : "dirección"
          }.`
        );
      ...
      await createClient(
        ...
        (createdCliente) => {
          if (createdCliente && createdCliente.clientId) {
            ...
            console.warn(
              "La respuesta del cliente no es válida:",
              createdCliente
            );
